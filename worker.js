...
// 心跳引擎一言 Cloudflare Worker

// 默认数据URL，实际使用时请通过环境变量设置
// 环境变量：DATAURL
...
// 初始化函数，获取数据
async function fetchHeartbeatData(env) {
  try {
    // 从环境变量获取数据URL或使用默认值
    const dataUrl = env.DATAURL || DEFAULT_DATA_URL;
...
async function handleRequest(request, env) {
  ...
  if (!heartbeatData) {
    try {
      heartbeatData = await fetchHeartbeatData(env);
    } catch (error) {
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      return createJsonResponse({ error: '服务器内部错误' }, 500);
    }
  }
};
